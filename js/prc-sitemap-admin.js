jQuery(document).ready(function () {
	...
	prc_query_sitemap_counts();

	function prc_query_sitemap_counts() {
		var data = {
			action: 'prc-sitemap-get-sitemap-counts',
		...
		jQuery.ajax({
			...
			data: data,
			dataType: 'json',
			success: prc_sitemap_counts_received,
		});

		// Refresh to get update counts
		setTimeout(prc_query_sitemap_counts, 60000);
	}

	function prc_sitemap_counts_received(response) {
		// Update the top stats
		jQuery('#sitemap-count').html(response['total_sitemaps']);
		jQuery('#sitemap-indexed-url-count').html(
			response['total_indexed_urls']
		);
		...
		for (var e in response['sitemap_indexed_urls']) {
			formatted_data.push([
				new Date(e).getTime(),
				response['sitemap_indexed_urls'][e],
				e,
			]);
	...
	placeholder.bind('plothover', function (event, pos, item) {
		if (item) {
			if (previous_point != item.dataIndex) {
				...
				var x = item.series.xaxis.tickFormatter(
					item.datapoint[0],
					item.series.xaxis
				);
				var y = item.series.yaxis.tickFormatter(
					item.datapoint[1],
					item.series.yaxis
				);
