<?php

if ( ! PRC_Sitemap::is_blog_public() ) {
	wp_die(
		__( 'Sorry, this site is not public so sitemaps are not available.', 'prc-sitemap' ),
		__( 'Sitemap Not Available', 'prc-sitemap' ),
		array( 'response' => 404 )
	);
}

$req_year = get_query_var( 'sitemap-year' );
if ( empty( $req_year ) ) {
	$req_year = ( isset( $_GET['yyyy'] ) ) ? intval( $_GET['yyyy'] ) : false;
}

$req_month = ( isset( $_GET['mm'] ) ) ? intval( $_GET['mm'] ) : false;
$req_day   = ( isset( $_GET['dd'] ) ) ? intval( $_GET['dd'] ) : false;

$build_xml = PRC_Sitemap::build_xml(
	array(
		'year'  => $req_year,
		'month' => $req_month,
		'day'   => $req_day,
	)
);

if ( $build_xml === false ) {
	wp_die(
		__( 'Sorry, no sitemap available here.', 'prc-sitemap' ),
		__( 'Sitemap Not Available', 'prc-sitemap' ),
		array( 'response' => 404 )
	);
}
header( 'Content-type: application/xml; charset=UTF-8' );
echo $build_xml;
